...
function App() {
  ...
  // isLive=true  -> /predict-live  (microphone recording)
  // isLive=false -> /predict       (uploaded file)
  const handleAnalyze = useCallback(async function(audio, isLive) {
    setIsAnalyzing(true);
    setResult(null);
    setErrorMsg(null);

    var endpoint = isLive ? "/predict-live" : "/predict";

    try {
      var formData = new FormData();
      var filename = audio && audio.name
        ? audio.name
      ...
      var displayProb = verdict === "real" ? data.bonafide_prob : data.spoof_prob;
        ...
        <AnimatePresence mode="wait">
          {mode === "record" && (
            <motion.div key="record"
              ...
              {/* passes isLive=true */}
              <AudioRecorder
                onRecordingComplete={function(blob) { handleAnalyze(blob, true); }}
          ...
          {mode === "upload" && (
            <motion.div key="upload"
              ...
              {/* passes isLive=false */}
              <FileUploader
                onFileSelect={function(file) { handleAnalyze(file, false); }}
                isAnalyzing={isAnalyzing} />
            </motion.div>
          )}
        </AnimatePresence>
