...
Fixes applied:
  ...
  3. Spectrogram instance normalization: makes amplitude irrelevant to the model
  4. Temperature scaling: honest confidence display
  5. Auto-download model from Google Drive (for Render deployment)
"""
...
MODEL_FILE_ID = "1EfI2FDUqsZsSYH0G8r0WsWlCFkJfDr8h"
...
def decode_uploaded_file(file_bytes):
    """Decode uploaded audio file using pydub. Works for wav/mp3/flac/ogg/webm."""
    audio = AudioSegment.from_file(io.BytesIO(file_bytes))
    audio = audio.set_channels(1).set_frame_rate(SAMPLE_RATE)
...
def predict_live():
    ...
    try:
        ...
        if not data or "samples" not in data:
            return jsonify({
                "error": "No samples received. Send JSON with 'samples' array."
            }), 400
